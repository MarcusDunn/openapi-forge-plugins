...
//! prune `Ir::types` down to a transitive-closure subset.

use forge_plugin_sdk::ir;
use std::collections::VecDeque;

/// Push every `TypeRef` directly referenced by `op` onto `queue`.
...
/// Push every `TypeRef` directly referenced by `def` (one level deep)
/// onto `queue`. The caller's BFS will discover transitive references.
