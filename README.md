...
| [`generator-html-docs`](generator-html-docs/) | `ghcr.io/marcusdunn/generator-html-docs:<tag>` | Emit a static HTML documentation site from an OpenAPI spec. Multi-page (one canonical page per endpoint), nested-tag sidebar (OAS 3.2 `tag.parent`), markdown descriptions, prominent deprecation banners, semantic HTML throughout (`<nav>`, `<main>`, `<article>`, `<dl>`). |
